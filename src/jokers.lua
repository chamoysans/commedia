                    ...
                    func = function()
                        if G.consumeables.config.card_limit > #G.consumeables.cards then
                            ...
                            local card = create_card(
                                (used_tarot.ability.name == 'The Emperor' and 'Tarot') or
                                (used_tarot.ability.name == 'The High Priestess' and 'Planet'), G.consumeables, nil, nil,
                                nil,
                                nil, nil,
                                (used_tarot.ability.name == 'The Emperor' and 'emp') or
                                (used_tarot.ability.name == 'The High Priestess' and 'pri'))
                            card:add_to_deck()
            ...
            if context.using_consumeable and (not context.CMDIA_dramatist) then
                ...
                if context.consumeable.ability.set == 'Planet' then
                        ...
                        update_hand_text({ sound = 'button', volume = 0.7, pitch = 0.8, delay = 0.3 },
                            {
                                ...
                                chips =
                                    G.GAME.hands[context.consumeable.ability.consumeable.hand_type].chips,
                                mult = G.GAME.hands
                                    [context.consumeable.ability.consumeable.hand_type].mult,
                                level = G.GAME.hands
                                    [context.consumeable.ability.consumeable.hand_type].level
                            })
                        level_up_hand(context.blueprint_card or card, context.consumeable.ability.consumeable.hand_type)
                    ...
                    SMODS.calculate_context({
                        ...
                        consumeable = context.consumeable,
                        area = context
                            .area,
                        CMDIA_dramatist = true,
                        CMDIA_comet = context.CMDIA_comet
                    })
                ...
                for i = 1, #G.jokers.cards do
                    if G.jokers.cards[i] == card then
                        if i < #G.jokers.cards then
                            ...
                            SMODS.calculate_effect({ message = CMDIA.get_dictionary("cmdia_locked") },
                                G.jokers.cards[i + 1])
                        end
                    end
                end
        ...
        calculate = function(self, card, context)
            if context.joker_main then
                ...
                for i = 1, #context.scoring_hand do
                    if (context.scoring_hand[i]:is_suit(valcard.suit1) or context.scoring_hand[i]:is_suit(valcard.suit2)) then
                        check =
                            check + 1
                    end
                end
    ...
    ['microchip'] = {
        ...
        calculate = function(self, card, context)
            ...
            if context.CMDIA.blind_modify_hand then
                ...
                SMODS.calculate_effect({ message = CMDIA.get_dictionary("cmdia_chipped") },
            ...
            if context.repetition and context.cardarea == G.play then
                ...
                if jt1 then
                    SMODS.calculate_effect(
                        { chips = cae.chips, message = localize { type = 'variable', key = 'a_chips', vars = { cae.chips } } },
                        card)
                end
                if jt2 then
                    SMODS.calculate_effect(
                        { mult = cae.mult, message = localize { type = 'variable', key = 'a_mult', vars = { cae.mult } } },
                        card)
                end
        ...
        calculate = function(self, card, context)
            ...
            if context.setting_blind and not context.blueprint then
                ...
                if entry[1] == "deal" or entry[1] == "oddball" then
                    ...
                    cae.quest_vars["hand"] = pseudorandom_element(hands,
                        pseudoseed("cmdia_qm_deal" .. tostring(cae.seedstuff)))
            ...
            if context.joker_main then
                if cae.buffs.chips > 0 then
                    SMODS.calculate_effect(
                        { chip_mod = buffs.chips, message = localize { type = 'variable', key = 'a_chips', vars = { buffs.chips } } },
                        card)
                end
                if cae.buffs.mult > 0 then
                    SMODS.calculate_effect(
                        { mult_mod = buffs.mult, message = localize { type = 'variable', key = 'a_mult', vars = { buffs.mult } } },
                        card)
                end
                if cae.buffs.xmult - 1 ~= 0 then
                    SMODS.calculate_effect(
                        { Xmult_mod = buffs.xmult, message = localize { type = 'variable', key = 'a_xmult', vars = { buffs.xmult } } },
                        card)
                end
            end
...
CMDIA.lib.addCycle(
    ...
    function(card)
        ...
        for k, v in ipairs({ 'Spades', 'Hearts', 'Clubs', 'Diamonds' }) do
            if (thingy and (v ~= card.suit2 and v ~= card.suit1) or (v ~= card.suit1)) then
                suits[#suits + 1] = v; thingy = 1
            end
        end
